(function () {
  ...
  const journeyProgress = { seed: '0%', roots: '33%', stem: '66%', bloom: '100%' };
  function setJourney(stage) {
    ...
    if (journeyRail && journeyProgress[stage]) {
      journeyRail.style.setProperty('--journey-progress', journeyProgress[stage]);
    }
  }
  if ('IntersectionObserver' in window && journeyLinks.length && journeyStages.length) {
  ...
  buildButtons.forEach((button) => {
    button.addEventListener('click', () => {
      buildButtons.forEach((item) => item.setAttribute('aria-selected', 'false'));
      button.setAttribute('aria-selected', 'true');
      if (buildPanel) buildPanel.textContent = button.getAttribute('data-build-copy');
    });
  });
  ...
  recordButtons.forEach((button) => {
    button.addEventListener('click', () => {
      recordButtons.forEach((item) => item.setAttribute('aria-selected', 'false'));
      button.setAttribute('aria-selected', 'true');
      if (recordPanel) {
        const title = button.getAttribute('data-record');
        const copy = button.getAttribute('data-record-copy');
        recordPanel.replaceChildren();
        const strong = document.createElement('strong');
        strong.textContent = title;
        const paragraph = document.createElement('p');
        paragraph.textContent = copy;
        recordPanel.append(strong, paragraph);
      }
    });
  });
})();
